...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        temp_path = os.path.join(
            data_dir, f"{basename}_{current_rank}_of_{total_ranks}.jsonl"
        )
        ...
        if os.path.exists(temp_path):
            ...
            lines_per_rank = (
                total_lines // total_ranks
            )  # Number of lines each rank should process
            lines_per_rank = max(1, lines_per_rank)

            # Calculate the start and end line numbers for the current rank
            start_line = (
                lines_per_rank * current_rank
            )  # Starting line for the current rank
    ...
    def _enable_worker_distributed(self):
        if (
            ...
            self.raw_data = self.raw_data[
                self.worker_id % num_worker_per_rank :: num_worker_per_rank
            ]
            logger.info(
